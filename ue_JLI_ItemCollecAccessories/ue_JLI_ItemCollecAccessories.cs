...
using ue_JLI_ItemCollecAccessories.Properties;

namespace ue_JLI_ItemCollecAccessories
{
    public class ue_JLI_ItemCollecAccessories : IDOExtensionClass
    {
        ...
        public int ue_JLI_AvailCollDataInsertAndDelete(string item,
                                                        string newstat,
                                                        string oldstat,                                                        
                                                        string newAssortGroups,
                                                        string oldAssortGroups,
                                                        string itemStat,
                                                        string itemLifeCycleStatus,
                                                        string priceBook,
                                                        string exclusive)
        {            
            ...
            if(newstat == "D")
            {
                ue_JLI_AvailCollDataDelete(item, newAssortGroups);
                return 0;
            }
            if (string.IsNullOrEmpty(itemLifeCycleStatus)) { itemLifeCycleStatus = "E-Ready"; }
            if (itemLifeCycleStatus != "E-Ready" || itemStat != "A" || newstat == "P")
                return 0;
            ...
            try
            {
                using (Mongoose.IDO.DataAccess.ApplicationDB db = this.CreateApplicationDB())
                {
                    ...
                    sqlCommand.CommandText = String.Format(query,item,newAssortGroups,oldAssortGroups,priceBook,exclusive);
            ...
            {
                //createLog("ue_JLI_ItemCollecAccessories", "ue_JLI_AvailCollDataInsertAndDelete", 139, ex.Message);
            }
            return 0;
        }
        public int ue_JLI_AvailCollDataDelete(string item, string assortGroups)
        {            
            string query =string.Empty;
            try
            {
            ...
            {
                //createLog("ue_JLI_ItemCollecAccessories", "ue_JLI_AvailCollDataDelete", 139, ex.Message);
            }
            return 0;
        }
